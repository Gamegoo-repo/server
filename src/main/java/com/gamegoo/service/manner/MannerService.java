...
public class MannerService {
    ...
    public MannerRating insertManner(MannerRequest.mannerInsertDTO request, Long memberId) {
        ...
        // 매너평가 최초 시도 여부 검증.
        List<MannerRating> mannerRatings = mannerRatingRepository.findByFromMemberIdAndToMemberId(member.getId(),
                targetMember.getId());
        List<MannerRating> positiveMannerRatings = mannerRatings
                .stream()
        ...
        List<MannerKeyword> mannerRatingKeywordList = request.getMannerRatingKeywordList()
                .stream()
        ...
        notificationService.createNotification(NotificationTypeTitle.MANNER_KEYWORD_RATED, mannerKeywordString, null,
                targetMember);
        ...
        if (targetMember.getMannerLevel() < mannerLevel) {
            Notification mannerUpNotification = notificationService.createNotification(
                    NotificationTypeTitle.MANNER_LEVEL_UP, mannerLevel.toString(), null, targetMember);
    ...
    public MannerRating insertBadManner(MannerRequest.mannerInsertDTO request, Long memberId) {
        ...
        Notification ratedNotification =
                notificationService.createNotification(NotificationTypeTitle.MANNER_KEYWORD_RATED,
        ...
        // 매너레벨 하락 알림 전송
        if (targetMember.getMannerLevel() > mannerLevel) {
            Notification mannerDownNotification =
                    notificationService.createNotification(NotificationTypeTitle.MANNER_LEVEL_DOWN,
                            mannerLevel.toString(), null, targetMember);
    ...
    public MannerRating update(MannerRequest.mannerUpdateDTO request, Long memberId, Long mannerId) {
        ...
        if (mannerRating.getIsPositive()) {
            ...
            // 매너레벨 상승 알림 전송
            if (targetMember.getMannerLevel() < mannerLevel) {
                Notification mannerUpNotification =
                        notificationService.createNotification(NotificationTypeTitle.MANNER_LEVEL_UP,
                                mannerLevel.toString(), null, targetMember);
                notificationRepository.save(mannerUpNotification);
    ...
    public void deleteMannerRatingsByMemberId(Long memberId) {
        ...
        for (Member member : toMembers) {
            ...
            if (member.getMannerLevel() < mannerLevel) {
                Notification mannerUpNotification =
                        notificationService.createNotification(NotificationTypeTitle.MANNER_LEVEL_UP,
                                mannerLevel.toString(), null, member);
                notificationRepository.save(mannerUpNotification);
    ...
    public Double getMannerScoreRank(Long memberId) {
        ...
        if (member.getMannerScore()==null) {
            ...
            long countHigherMannerScores =
                    memberRepository.countByMannerScoreGreaterThanAndBlindFalse(member.getMannerScore());
